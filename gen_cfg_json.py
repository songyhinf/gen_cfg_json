...
if __name__ == '__main__':
    ...
    JSON_FOLDER_PATH = idaapi.get_plugin_options("json")

    binary_file_dict = analyze_binary_file()

    input_file_path = idc.get_input_file_path()
    input_file_name = input_file_path.split('\\')[-1]
    JSON_PATH = os.path.join(JSON_FOLDER_PATH, input_file_name + '.json')

    with open(JSON_PATH, "w") as f:
        json.dump({input_file_path: binary_file_dict}, f, indent=4)
